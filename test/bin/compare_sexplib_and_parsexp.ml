...
module Cst_to_sexp_with_layout = struct
  ...
  let convert_pos ?(delta = 0) (pos : Parsexp.Positions.pos) : Sexplib.Src_pos.Absolute.t =
    ...
    { col = pos.col + 1 + delta; row = pos.line }
  ;;
  ...
  and convert_comment : Src.comment -> Dst.comment = function
    | Plain_comment { loc; comment } -> Plain_comment (convert_pos loc.start_pos, comment)
    | Sexp_comment { hash_semi_pos; comments; sexp } ->
      Sexp_comment
        (convert_pos hash_semi_pos, List.map comments ~f:convert_comment, convert sexp)
  ;;
end
...
let test_one_kind ~ascii ~what ~sexplib_load ~parsexp_load fn ~sexp_of:sexp_of_a =
  ...
  let fail () =
    ...
    let (_exit_code : int) =
      ksprintf
        Sys_unix.command
        "patdiff %s sexplib.sexp.tmp parsexp.sexp.tmp"
...
let test fn ~ascii =
  ...
  test_one_kind
    ~ascii
    ~what:"the normal parser vs sexplib"
    ~sexplib_load:(try_withx Sexplib.Sexp.load_sexps)
    ~parsexp_load:
      (try_withx (fun filename -> Parsexp_io.load_exn (module Parsexp.Many) ~filename))
    ~sexp_of:Fn.id
    fn;
  test_one_kind
    ~ascii
    ~what:"the cst parser vs sexplib with layout"
    ~sexplib_load:
      (try_withx (fun filename ->
  ...
  test_one_kind
    ~ascii
    ~what:"the normal parser vs the cst one"
    ~sexplib_load:
      (try_withx (fun filename -> Parsexp_io.load_exn (module Parsexp.Many) ~filename))
    ~parsexp_load:
      (try_withx (fun filename ->
...
let rec walk dir ~f =
  Array.iter (Sys_unix.readdir dir) ~f:(fun fn ->
    ...
    if Sys_unix.is_directory full_fn = `Yes
    then walk full_fn ~f
    else if fn = "jbuild" || String.is_suffix fn ~suffix:".sexp"
    then f full_fn)
;;
...
let () =
  Command_unix.run
    (Command.basic
       ~summary:"compare Parsexp and Sexplib on the whole tree"
       (let open Command.Let_syntax in
        let%map_open ascii = flag "-ascii" no_arg ~doc:" disable colors"
        and root = flag "-root" (required string) ~doc:" repo root" in
        fun () ->
